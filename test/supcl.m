...
for ind=1:3%length(X1)
	...
	[~,loc]=min(t);
    corr=imresize(Y(index(loc)).phase,sz);
	drate=-imresize(k,sz);
	centers=computecenters(coords,amp,corr,drate);
	[c,min_dis]=classification(centers,amp,corr,drate);
	L1(ind).c=c;
	L1(ind).mindis=min_dis;
	[~,latmin,latmax,lonmin,lonmax]=fitkml3(L1(ind).c,nr,folder,'savefilename',strcat(char(savefolder),'/',char(X1(ind).id)),'marks',colormarks,'cutedge',[150,150,100,100]);
end
